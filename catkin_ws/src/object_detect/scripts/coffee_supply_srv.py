...
class RealSenseCamera:
    def __init__(self):
        ...
        try:
            ...
            self.pipeline.start(config)
            self.started = True
            rospy.loginfo("RealSense Camera Initialized (RGB only, 640x360).")
        except Exception as e:
            rospy.logerr(f"[Camera] Failed to start RealSense pipeline: {e}")
    ...
    def get_color_image(self):
        try:
            ...
            rospy.logerr(f"[Camera] Error getting frame: {e}")
            return None
    ...
    def detect(self, img_color):
        ...
        for r in results:
            ...
            for box, cls_id in zip(boxes, classes):
                ...
                cx = (x1 + x2) / 2
                cy = (y1 + y2) / 2
                detections.append({'class_name': r.names[int(cls_id)], 'xy': (cx, cy)})
        return detections

# =============================================================================
# Class: App (Service Handler)
# =============================================================================
class App:
    def __init__(self, model_path, cup_color_model_path):
        rospy.loginfo("Initializing Coffee Supply Server ......")
    ...
    def coffee_command(self, req):
        ...
        try:
            ...
            # 假設 target_name 是最常出現的物件類別
            names = [d['class_name'] for d in detections_all if 'class_name' in d]
            target_name = max(set(names), key=names.count)
            # 取第一個座標作為位置
            first_det = next((d for d in detections_all if d['class_name'] == target_name), None)
            if first_det:
                self.positions[target_name] = first_det['xy']
            ...
            cup_side = 'unknown'
            if 'cup' in cup_positions:
                cup_side = cup_positions['cup']

            rospy.loginfo(f"Detection complete: {target_name}, cup_side: {cup_side}")
            return DetectCoffeeSupplyResponse(success=True, target_name=target_name, table=1, cup_side=cup_side)
    ...
    def shutdown(self):
        rospy.loginfo("Shutting down Coffee Supply Server...")
        if self.camera:
            self.camera.stop()
...
# =============================================================================
if __name__ == "__main__":
    rospy.init_node('coffee_supply_server')
    model_path = rospy.get_param('~model_path', '/root/models/coffee.pt')
    cup_color_model_path = rospy.get_param('~cup_color_model_path', '/root/models/cup_color.pt')
    app = App(model_path, cup_color_model_path)
